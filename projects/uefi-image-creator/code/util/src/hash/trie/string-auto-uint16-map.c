...
flo_NewStringInsert flo_trie_insertStringAutoUint16Map(
    ...
    for (U64 hash = flo_hashStringDjb2(key); *currentNode != NULL;
         hash <<= 2) {
    ...
    *currentNode =
        NEW(perm, flo_trie_StringAutoUint16Node, 1, ZERO_MEMORY);
...
U16
flo_trie_containsStringAutoUint16Map(string key,
