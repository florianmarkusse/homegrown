...
typedef struct DevicePathUtilitiesProtocol {
    USize(*get_device_path_size)(DevicePathProtocol *device_path);
    DevicePathProtocol *(*duplicate_device_path)(
        DevicePathProtocol *device_path);
    DevicePathProtocol *(*append_device_path)(DevicePathProtocol *src1,
                                                      DevicePathProtocol *src2);
    DevicePathProtocol *(*append_device_node)(
        DevicePathProtocol *device_path, DevicePathProtocol *device_node);
    DevicePathProtocol *(*append_device_path_instance)(
        DevicePathProtocol *device_path,
        DevicePathProtocol *device_path_instance);
    DevicePathProtocol *(*get_next_device_path_instance)(
        DevicePathProtocol **device_path_instance,
        USize *device_path_instance_size);
    bool(*is_device_path_multi_instance)(
        DevicePathProtocol *device_path);
    DevicePathProtocol *(*create_device_node)(U8 node_type,
                                                      U8 node_subtype,
                                                      U16 node_length);
