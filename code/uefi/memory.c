...
typedef CEfiU32 __attribute__((aligned(1))) u32;
typedef CEfiU64 __attribute__((aligned(1))) u64;
...
#ifdef __GNUC__
typedef __attribute__((__may_alias__)) CEfiU64 WT;
#define WS (sizeof(WT))
#endif
...
__attribute((nothrow, nonnull(1, 2))) void *memmove(void *dest, const void *src,
    ...
    if ((CEfiI64)((CEfiU64)s - (CEfiU64)d - n) <= -2 * n) {
        return memcpy(d, s, n);
    }

    if (d < s) {
#ifdef __GNUC__
        if ((CEfiU64)s % WS == (CEfiU64)d % WS) {
            while ((CEfiU64)d % WS) {
                if (!n--) {
                    return dest;
                }
            ...
            for (; (CEfiU64)n >= WS; n -= WS, d += WS, s += WS) {
                *(WT *)d = *(WT *)s;
            }
        }
#endif
...
static inline void *small_memset(void *s, int c, CEfiU64 n) {
    if (n < 5) {
        if (n == 0)
            return s;
        char *p = s;
    ...
    if (n <= 16) {
        CEfiU64 val8 = ((CEfiU64)0x0101010101010101L * ((CEfiU8)c));
        if (n >= 8) {
            char *first = s;
            char *last = s + n - 8;
            *((u64 *)first) = val8;
...
static inline void *huge_memset(void *s, int c, CEfiU64 n) {
    char *p = s;
    char X = (char)c;
    ...
    *((char32 *)p) = val32;
    ...
    char *first_aligned = p + 32 - ((CEfiU64)p % 32);
    char *buffer_end = p + n;
...
memcmp(const void *s1, const void *s2, CEfiI64 n) {
    ...
    for (i = 0; (i < (CEfiU64)n) && (*(CEfiU8 *)s1 == *(CEfiU8 *)s2);
         i++, s1 = 1 + (CEfiU8 *)s1, s2 = 1 + (CEfiU8 *)s2)
        ;

    // if i == length, then we have passed the test
    return (i == (CEfiU64)n) ? 0 : (*(CEfiU8 *)s1 - *(CEfiU8 *)s2);
}
