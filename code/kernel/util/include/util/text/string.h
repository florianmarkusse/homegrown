...
__attribute__((unused)) static inline unsigned char flo_getChar(flo_string str,
                                                                int64_t index) {
...
flo_getCharOr(flo_string str, int64_t index, char or) {
...
__attribute__((unused)) static inline unsigned char *
flo_getCharPtr(flo_string str, int64_t index) {
    FLO_ASSERT(index >= 0 && index < str.len);
...
__attribute__((unused)) static inline flo_string
flo_splitString(flo_string s, unsigned char token, int64_t from) {
    FLO_ASSERT(from >= 0 && from < s.len);

    for (int64_t i = from; i < s.len; i++) {
        if (s.buf[i] == token) {
...
flo_firstOccurenceOfFrom(flo_string s, unsigned char ch, int64_t from) {
    FLO_ASSERT(from >= 0 && from < s.len);
